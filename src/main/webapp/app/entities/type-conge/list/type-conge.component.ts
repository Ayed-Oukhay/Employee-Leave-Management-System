...
export class TypeCongeComponent implements OnInit {
  ...
  trackId(index: number, item: ITypeConge): number {
    return item.id!;
  }

  delete(typeConge: ITypeConge): void {
  ...
  protected onSuccess(data: ITypeConge[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/type-conge'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
